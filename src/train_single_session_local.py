"""
train_single_session_local.py
Fine-tuning on one local session with neuron masking only.
"""
...
class LocalSessionDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        sample = {
            # Each item is a single [time, neuron] spike-count window.
...
def collate_local_batch(batch):
    """
    Keep eid as list[str].
    Keep neuron_regions as np.ndarray of shape [B, N] (NOT a tensor).
    """
    out = {
...
def extract_state_dict(checkpoint_obj):
    # state-dict checkpoints so the local trainer can reuse whichever
    # pretrained artifact is available.
...
def load_pretrained_weights(model, checkpoint_path: str, device: torch.device) -> dict:
    # Load the checkpoint and extract the state dict, 
    ...
    for key, value in checkpoint_state.items():
        ...
        # Loading tensors whose names and shapes still match.
    ...
    # Returning records about what was loaded and skipped and 
    # report run artifacts that were reused. 
    return {
...
def cfg_set(config, dotted_path: str, value) -> None:
    # DictConfig.__getattr__ wraps nested dicts in copies, so assignments like
    # config.model.encoder.foo = x  do not randomly change the real config.
    cur = config
...
def build_config(args):
    """
    Builds a single-session fine-tuning config that stays close to the
    ...
    # Keeps stitching enabled so the model still uses the pretrained shared
    # latent neuron space 
    ...
    # Keep the pretrained shared stitched width; only time length is data-specific.
...
def make_model(config, num_neurons: int):
    meta_data = {
        # Even with one session, stitching still needs to know the raw neuron
        # count so it can build the session-specific input/output projections.
...
def run_epoch(model, loader, optimizer, device, train: bool, max_batches: int = None, lr_scheduler=None):
    ...
    for step, batch in enumerate(loader):
        ...
        with torch.set_grad_enabled(train):
            ...
            # `outputs.loss` is the summed Poisson NLL over masked bins only.
            # `outputs.n_examples` is the number of masked bins, so the script
            # reports a masked-bin average at the end of the epoch.
            loss = outputs.loss
...
def main():
    ...
    parser.add_argument("--save-dir", type=str, default="results_20ms/local_neuron_mask")
    ...
    parser.add_argument("--eval-mask-ratio", type=float, default=None,
                        help="Held-out neuron ratio for artifact evaluation; defaults to --mask-ratio")
    ...
    # Loading as much of the pretrained IBL MtM checkpoint as still matches the
    # local single-session fine-tuning model.
    ...
    # Smoke-test one batch before full training
   ...
   # one forward pass - smoke test sanity testing
    ...
    # Save run metadata before the long loop 
    # If say a job fails mid-run
    save_run_metadata(
        args.save_dir,
        args,
        config,
    ...
    # Keeping structured history in memory and flush it every epoch for safe plotting later.
    history = []
    training_start_time = time.perf_counter()
    # Early stopping is intentionally based on validation loss, not train loss in this code,
    # and is disabled by default
    epochs_since_improvement = 0

    for epoch in range(args.epochs):
        epoch_start_time = time.perf_counter()
        # Training and validation are both evaluated in terms of average loss
        # per masked bin so runs with different mask counts remain comparable.
        ...
        # Save `last.pt` after best-val is recorded to keep everything consistent
        save_checkpoint(args.save_dir, "last", model, optimizer, epoch, best_val_loss)

        # CSV/JSON/PNG for artifacts plotted later 
        history.append({
            "epoch": epoch,
    ...
    if not args.skip_artifacts:
        # Reload the best checkpoint so final plots/tables correspond to model selection. 
