"""
train_single_session_local_ndt1_stitched_scratch.py

Trains a single-session NDT1 from scratch with the stitched architecture.
"""
...
def build_config(args):
    """
    Builds a stitched single-session NDT1 config trained from scratch.

    Keeps the stitched architecture and, by default, the prompt token as well.
    The only thing missing now is the pretrained checkpoint.
    """
    kwargs = {
    ...
    cfg_set(config, "wandb.project", "single-session-local-ndt1-stitched-scratch")
    ...
    # Keep the stitched shared space so this baseline stays architecturally close
    # to the transferred MtM runs.
    ...
    cfg_set(config, "model.encoder.embedder.use_prompt", not args.no_prompt)
    ...
    if args.masking_mode == "causal":
        # Match the local causal runs: temporal masking plus causal context.
        ...
        # Match the local neuron runs: mask neurons while leaving full temporal context.
...
def main():
    ...
    parser.add_argument("--masking-mode", type=str, default="neuron", choices=["neuron", "causal"],
                        help="Objective to train in the stitched scratch control")
    parser.add_argument("--no-prompt", action="store_true",
                        help="Disable the prompt token; by default this stitched baseline keeps the prompt token on")
    parser.add_argument("--batch-size", type=int, default=8)
    parser.add_argument("--epochs", type=int, default=100)
    parser.add_argument("--lr", type=float, default=5e-5,
                        help="Single LR for all trainable parameters in the stitched scratch baseline")
    ...
    print("[BASELINE] stitched single-session NDT1 scratch")
