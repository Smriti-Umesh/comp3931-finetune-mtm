"""
train_single_session_local_ndt1_scratch_combined.py

Trains a single-session NDT1 from scratch with the
combined neuron and causal objective.
"""
...
def build_config(args):
    """
    Direct NDT1 from scratch with combined (neuron + causal) objective.

    No stitching, no prompt, no session token — the minimal NDT1 architecture.
    Attention is fixed to causal (matching mtm_combined) so both masking modes
    train under the same attention pattern.  The masker mode is overridden
    per-batch during training; the config default here is just a placeholder.
    """
    # Causal yaml sets the right attention context (forward=0); overriding the
    # masker mode per batch so the yaml choice only affects attention, not masking.
    kwargs = {"model": "include:src/configs/ndt1_causal.yaml"}
    config = config_from_kwargs(kwargs)
    config = update_config("src/configs/ndt1_causal.yaml", config)
    ...
    cfg_set(config, "wandb.project", "single-session-local-ndt1-scratch-combined")
    ...
    # Direct NDT1: no stitching, no prompt, no session.
    ...
    # Causal attention context (same as mtm_combined).
    cfg_set(config, "model.encoder.context.forward", 0)
    cfg_set(config, "model.encoder.context.backward", -1)

    # Masker config — mode will be overridden per batch; ratio set at runtime.
    cfg_set(config, "model.encoder.masker.force_active", True)
    cfg_set(config, "model.encoder.masker.mode", "neuron")  # placeholder
...
def main():
    ...
    print("[BASELINE] direct NDT1 scratch — combined neuron+causal objective")
    ...
    load_report = {
        "mode": "scratch_direct_ndt1_combined",
    ...
    # Sanity-checking both modes before training.
